...
public class OutOfStructureFilter extends PlacementModifier {
    ...
    public Stream<BlockPos> getPositions(PlacementContext worldDecoratingHelper, Random random, BlockPos blockPos) {
        if (!(worldDecoratingHelper.getLevel().getLevel().getChunkSource().getGenerator() instanceof ChunkGeneratorEdge teChunkGen))
            return Stream.of(blockPos);
        
        // Feature Center
        BlockPos.MutableBlockPos featurePos = TEFeature.getNearestCenterXYZ(blockPos.getX() >> 4, blockPos.getZ() >> 4).mutable();
        
        final TEFeature feature = teChunkGen.getFeatureCached(new ChunkPos(featurePos), worldDecoratingHelper.getLevel());
    ...
    public PlacementModifierType<?> type() {
        return TheEdgeFeatures.NO_STRUCTURE.get();
    }
